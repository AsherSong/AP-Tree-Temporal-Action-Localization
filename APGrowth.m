function out=APGrowth(T,MTST,MMST)

    % Step 1: Create Sorted Items List, in order of Descending Frequency
    Items=[];
    for i=1:numel(T)
        tempitems=[];
        tempitems=union(tempitems,T{i});
        for j=1:numel(tempitems)
            if length(find(tempitems(j)==T{i}))/length(T{i})<MTST
                tempitems(j)=0;
            end
        end
        tempitems(find(tempitems==0))=[];
        Items=union(Items,tempitems);  
    end
    Items=reshape(Items,1,[]);       
               
    Count=zeros(size(Items));
    for i=1:numel(T)
        Count=Count+ismember(Items,T{i});%统计次数
    end
    allsum=sum(Count)*MMST;
    for i=1:numel(Count)
        if Count(i) < allsum
            Count(i)=0;
            Items(i)=0;
    ...
    Patterns=Patterns(PatternCount/numel(T)>=MTST);
    PatternCount=PatternCount(PatternCount/numel(T)>=MTST);
    ...
    FinalRules=Rules(Conf>=MMST & Lift>=1,:);
